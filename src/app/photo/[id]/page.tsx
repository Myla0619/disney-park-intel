      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        {"source" in spot && (spot as any).source?.url && (
          <a
            ...
            className="flex items-start gap-2 rounded-xl border border-castle-400/25 bg-castle-500/15 p-3 text-xs text-castle-100/90 transition-colors hover:bg-castle-500/15"
          >
            <ExternalLink className="mt-0.5 h-3.5 w-3.5 shrink-0 text-castle-300" />
            <span>
              该机位来自游客笔记，非官方信息。
              {(spot as any).source.quote && (
                <span className="mt-1 block text-castle-100/60">
                  「{String((spot as any).source.quote).slice(0, 60)}」
                </span>
              )}
              <span className="mt-1 block text-castle-300 underline">查看原帖 →</span>
            </span>
          </a>
        )}
        ...
        <div className="flex gap-2 flex-wrap">
          <span className="text-xs bg-spark-500/20 text-spark-300 px-2.5 py-1 rounded-full">{typeLabel}</span>
        ...
        <div className="bg-night-800/55 rounded-xl p-4">
          <h2 className="font-medium text-sm text-white mb-3 flex items-center gap-2">
            <Clock className="w-4 h-4 text-castle-400" /> 最佳拍摄时段
          </h2>
          <div className="space-y-2">
            {spot.bestTimeSlots.map((slot, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-castle-400 flex-shrink-0" />
        ...
        <div className="bg-night-800/55 rounded-xl p-4">
          <h2 className="font-medium text-sm text-white mb-2 flex items-center gap-2">
            <MapPin className="w-4 h-4 text-magic-400" /> 位置信息
          </h2>
        ...
        {/* 拍摄技巧 */}
        <div className="bg-spark-500/10 border border-spark-500/20 rounded-xl p-4">
          <h2 className="font-medium text-sm text-spark-300 mb-2 flex items-center gap-2">
        ...
        <a href={`https://www.xiaohongshu.com/search_result?keyword=${encodeURIComponent(spot.xhsKeyword)}`}
           ...
           className="flex items-center justify-between p-4 bg-ember-500/10 border border-ember-500/20 rounded-xl hover:bg-ember-500/15 transition-all">
          <div>
            <div className="text-ember-400 font-medium text-sm">小红书更多机位参考</div>
