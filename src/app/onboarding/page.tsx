 ...
 * 每个游玩模式一个主题色。
 *
 * 原来五张卡片选中前完全一样，是一片灰。给各自的识别色之后，选择本身就带信息量，
 * 且这个色会贯穿到后面的行程卡片，形成一致的视觉线索。
 */
const MODES = [
  { id:"family",   label:"带娃家庭", desc:"亲子优先，适合所有年龄", icon:Users,
    ring:"border-castle-400", fill:"bg-castle-500/25", ink:"text-castle-300", glow:"shadow-[0_8px_28px_-8px_rgba(251,191,36,.55)]" },
  { id:"thrill",   label:"只玩刺激", desc:"过山车、速度、肾上腺素", icon:Zap,
    ring:"border-spark-400", fill:"bg-spark-500/25", ink:"text-spark-300", glow:"shadow-glow-spark" },
  { id:"casual",   label:"轻松游览", desc:"经典项目，不排超长队",   icon:Coffee,
    ring:"border-lagoon-400", fill:"bg-lagoon-500/25", ink:"text-lagoon-300", glow:"shadow-[0_8px_28px_-8px_rgba(34,211,238,.55)]" },
  { id:"photo",    label:"拍照打卡", desc:"美景、城堡、ins风景点",  icon:Camera,
    ring:"border-magic-400", fill:"bg-magic-500/25", ink:"text-magic-300", glow:"shadow-glow" },
  { id:"shopping", label:"购物美食", desc:"边逛边吃，打卡周边商品", icon:ShoppingBag,
    ring:"border-meadow-400", fill:"bg-meadow-500/25", ink:"text-meadow-400", glow:"shadow-[0_8px_28px_-8px_rgba(52,211,153,.5)]" },
] as const;
...
export default function OnboardingPage() {
  ...
  const Toggle = ({ value, onChange, label }: { value:boolean; onChange:()=>void; label:string }) => (
    <div className="flex items-center gap-3">
      <button onClick={onChange} className={`w-10 h-6 rounded-full transition-all flex-shrink-0 ${value?"bg-magic-500":"bg-white/20"}`}>
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-castle-gradient">迪士尼乐园智能助手</h1>
          <p className="text-magic-200 mt-1 text-sm">AI Agent · TSP 最优动线 · 历史数据预测</p>
        </div>

        <div className="flex gap-2 mb-6">
          {[1,2,3,4].map((s) => (
            <div key={s} className={`h-1 flex-1 rounded-full transition-all ${s<=step?"bg-magic-400":"bg-white/10"}`} />
          ))}
        </div>
        ...
        <div className="card-night p-6">
          ...
          {step===1 && (
            <div>
              ...
              <p className="text-magic-200 text-sm mb-4">目前支持：上海迪士尼乐园</p>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {MODES.map((m) => (
                  <button key={m.id} onClick={() => setMode(m.id as any)}
                    className={`p-3 rounded-xl border-2 text-left transition-all ${mode===m.id?`${m.ring} ${m.fill} ${m.glow}`:"border-white/10 bg-white/5 hover:border-white/25 hover:bg-white/[0.07]"}`}>
                    {/* 未选中也保留主题色（降透明度），否则五张卡片选前是一片灰，
                        整套配色要到点击之后才出现 */}
                    <m.icon className={`w-4 h-4 mb-1.5 ${m.ink} ${mode===m.id?"":"opacity-45"}`} />
              ...
              <input type="date" value={date} min={new Date().toISOString().slice(0,10)}
                ...
                className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-magic-400 mb-4" />
              <div className="text-center text-white/20 text-xs mb-4">
                东京 · 香港 · 巴黎 · 美国各园区 — 即将上线
              </div>
              <button disabled={!mode} onClick={() => setStep(2)}
                className="btn-magic flex w-full items-center justify-center gap-2 py-3">
          ...
          {step===2 && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-magic-200 text-sm">帮助 AI 精确规划全天行程</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={arrival} onChange={(e) => setArrival(e.target.value)}
                    className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-magic-400">
                    {HOURS.map(h => <option key={h} value={h} className="bg-night-800">{h}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={departure} onChange={(e) => setDeparture(e.target.value)}
                    className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-magic-400">
                    {HOURS.filter(h=>h>arrival).map(h => <option key={h} value={h} className="bg-night-800">{h}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2 flex-wrap">
                  {kids.map((kid,i) => (
                    <span key={i} className="flex items-center gap-1 bg-magic-500/20 text-magic-200 text-xs px-2.5 py-1 rounded-full">
                ...
                <div className="flex gap-2">
                  <input type="number" placeholder="年龄" value={kidAge} min={0} max={17}
                    ...
                    className="w-20 bg-white/10 border border-white/20 rounded-lg px-2 py-2 text-white text-sm placeholder-white/30 focus:outline-none focus:border-magic-400" />
                  <input type="number" placeholder="身高cm" value={kidHeight} min={50} max={200}
                    ...
                    className="flex-1 bg-white/10 border border-white/20 rounded-lg px-2 py-2 text-white text-sm placeholder-white/30 focus:outline-none focus:border-magic-400" />
              ...
              <div>
                ...
                <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
                  {LL_PACKAGES.map((pkg) => (
                    <button key={pkg.id} onClick={() => setLLPackage(pkg.id)}
                      className={`w-full px-3 py-2.5 rounded-xl border text-left transition-all ${llPackage===pkg.id?"border-castle-400 bg-castle-500/20":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                      <div className="flex justify-between items-start">
                        <span className={`font-medium text-sm ${llPackage===pkg.id?"text-white":"text-white/70"}`}>{pkg.name}</span>
                        <span className="text-xs text-castle-400 flex-shrink-0 ml-2">{pkg.price}</span>
                      </div>
                      ...
                      {(pkg.hasReservedParade || pkg.hasReservedFireworks) && (
                        <div className="flex gap-1 mt-1">
                          {pkg.hasReservedParade && <span className="text-xs bg-spark-500/20 text-spark-300 px-1.5 py-0.5 rounded">🎠 花车预留位</span>}
                          {pkg.hasReservedFireworks && <span className="text-xs bg-castle-500/20 text-castle-300 px-1.5 py-0.5 rounded">🎆 烟花预留位</span>}
                          {pkg.hasVIPEntrance && <span className="text-xs bg-magic-500/20 text-magic-300 px-1.5 py-0.5 rounded">🚀 快速入园</span>}
                        </div>
                      )}
                    </button>
                  ))}
                ...
                {/* 单项自选 */}
                {llPackage==="single" && (
                  <div className="mt-3 p-3 bg-castle-500/12 rounded-xl border border-castle-400/25">
                    <p className="text-castle-300 text-xs mb-2">选择购买了单项尊享卡的项目：</p>
                    <div className="space-y-1">
                      {llRides.map((ride) => (
                        <button key={ride.id} onClick={() => setSingleRides((p) => p.includes(ride.id)?p.filter(r=>r!==ride.id):[...p,ride.id])}
                          className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left text-xs transition-all ${singleRides.includes(ride.id)?"bg-castle-500/25 text-white":"bg-white/5 text-white/50 hover:text-white/70"}`}>
                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${singleRides.includes(ride.id)?"border-castle-400 bg-castle-400":"border-white/30"}`}>
                            {singleRides.includes(ride.id) && <span className="text-night-950 text-xs font-bold">✓</span>}
                          </div>
                          {ride.name}
                ...
                {llPackage==="bundle3" && (
                  <div className="mt-3 p-3 bg-castle-500/12 rounded-xl border border-castle-400/25">
                    <p className="text-castle-300 text-xs mb-2">选择3个项目（{bundle3Rides.length}/3）：</p>
                    <div className="space-y-1">
                      {llRides.map((ride) => {
                        ...
                        return (
                          <button key={ride.id} disabled={disabled}
                            ...
                            className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left text-xs transition-all ${sel?"bg-castle-500/25 text-white":disabled?"text-white/20":"bg-white/5 text-white/50 hover:text-white/70"}`}>
                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${sel?"border-castle-400 bg-castle-400":"border-white/30"}`}>
                              {sel && <span className="text-night-950 text-xs font-bold">✓</span>}
                            </div>
                            {ride.name}
                          </button>
                        );
                ...
                <div className="grid grid-cols-3 gap-2">
                  {DINING_PREFS.map((d) => (
                    <button key={d.id} onClick={() => setDiningPref(d.id as any)}
                      className={`p-2 rounded-xl border-2 text-center transition-all ${diningPref===d.id?"border-castle-400 bg-castle-500/20":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                      <d.icon className={`w-4 h-4 mx-auto mb-1 ${diningPref===d.id?"text-castle-300":"text-white/40"}`} />
                  ...
                  {watchParade && (
                    <div className="mt-2 ml-14 space-y-1.5">
                      ...
                      {selectedPkg?.hasReservedParade && (
                        <p className="text-spark-300 text-xs">✓ 你的套餐含花车预留观赏区，凭套票前往指定区域即可</p>
                      )}
                    </div>
                  )}
                </div>
                  ...
                  {watchFireworks && (
                    <div className="mt-2 ml-14 space-y-1.5">
                      ...
                      {selectedPkg?.hasReservedFireworks && (
                        <p className="text-castle-300 text-xs">✓ 你的套餐含烟花预留观赏区，凭套票前往指定区域即可</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => setStep(3)} className="btn-magic flex flex-1 items-center justify-center gap-2 py-3">
                  下一步 <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          {step===3 && (
            <div>
              ...
              <p className="text-magic-200 text-sm mb-4">影响算法在「排队」和「步行」之间的权衡</p>
              <div className="space-y-3 mb-6">
                {ROUTE_PROFILES.map((rp) => (
                  <button key={rp.id} onClick={() => setRouteProfile(rp.id as any)}
                    className={`w-full p-4 rounded-xl border-2 text-left transition-all ${routeProfile===rp.id?"border-magic-400 bg-magic-500/20":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                    <div className="flex items-center gap-3">
                      <rp.icon className={`w-5 h-5 flex-shrink-0 ${routeProfile===rp.id?"text-magic-300":"text-white/40"}`} />
                      <div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setStep(4)} className="btn-magic flex flex-1 items-center justify-center gap-2 py-3">
                  下一步 <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          {step===4 && (
            <div>
              ...
              <p className="text-magic-200 text-sm mb-4">可选，选了会影响行程安排</p>

              <div className="grid grid-cols-2 gap-3 mb-4">
                <button onClick={() => setFocusPhoto(!focusPhoto)}
                  className={`p-3 rounded-xl border-2 text-left transition-all ${focusPhoto?"border-spark-400 bg-spark-500/20":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                  <Camera className={`w-5 h-5 mb-1.5 ${focusPhoto?"text-spark-300":"text-white/40"}`} />
                ...
                <button onClick={() => setFocusShopping(!focusShopping)}
                  className={`p-3 rounded-xl border-2 text-left transition-all ${focusShopping?"border-meadow-400 bg-meadow-500/20":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                  <ShoppingBag className={`w-5 h-5 mb-1.5 ${focusShopping?"text-meadow-400":"text-white/40"}`} />
                ...
                {restaurants.map((r) => {
                  ...
                  return (
                    ...
                    <button
                      ...
                      className={`w-full p-3 rounded-xl border text-left transition-all ${sel?"border-castle-400 bg-castle-500/15":"border-white/10 bg-white/5 hover:border-white/20"}`}>
                      <div className="flex justify-between items-start gap-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`font-medium text-sm ${sel?"text-white":"text-white/80"}`}>{r.name}</span>
                            {r.requiresReservation && <span className="text-xs bg-ember-500/20 text-ember-400 px-1.5 py-0.5 rounded">需预约</span>}
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                          ...
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${sel?"border-castle-400 bg-castle-400":"border-white/30"}`}>
                            {sel && <span className="text-night-950 text-xs font-bold">✓</span>}
                          </div>
                        </div>
                      </div>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button onClick={handleFinish}
                  className="btn-magic flex flex-1 items-center justify-center gap-2 py-3">
        ...
        <p className="text-center text-white/20 text-xs mt-4">
          数据来源：themeparks.wiki · Queue-Times.com · 小红书 · TripAdvisor
        </p>
      </div>
    </div>
  );
