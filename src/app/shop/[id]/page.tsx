    ...
    <div className="min-h-screen bg-night-900 text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        <div className="flex gap-2 flex-wrap">
          {shop.hasLimitedEdition && <span className="text-xs bg-castle-500/20 text-castle-300 px-2.5 py-1 rounded-full">⭐ 有限定款</span>}
        ...
        <div className="bg-night-800/55 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-sm"><MapPin className="w-4 h-4 text-magic-400" /><span className="text-white/70">{shop.areaName}</span></div>
          <div className="flex items-center gap-2 text-sm"><Clock className="w-4 h-4 text-castle-400" /><span className="text-white/70">建议时机：{bestTimeLabel}</span></div>
          <div className="flex items-center gap-2 text-sm"><Tag className="w-4 h-4 text-meadow-400" /><span className="text-white/70">主题：{shop.theme}</span></div>
          <p className="text-white/50 text-xs">建议停留：{shop.duration} 分钟</p>
        </div>
        <div className="bg-meadow-500/10 border border-meadow-500/20 rounded-xl p-4">
          <h2 className="font-medium text-sm text-meadow-400 mb-2">购物小贴士</h2>
