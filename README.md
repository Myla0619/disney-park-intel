...
| 评论 | 小红书离线采集语料（`data/reviews/`） | 语料为空时用 `seed-reviews.ts` 人工示例，响应 `fallback: true` |
| 项目评分 | Claude 结构化输出 | 无 API key 或调用失败时用本地规则评分，响应 `fallback: true` |
| AI 助手 | Claude Tool Use，SSE 流式 | 无 API key 时返回 503，前端如实提示 |

**当前仓库状态**：`data/reviews/` 尚未采集（需要 Apify token，见下文），因此评论走的是
人工示例数据，接口会以 `fallback: true` 标注，Agent 也被提示词要求向用户说明这是示例内容。
